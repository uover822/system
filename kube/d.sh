:
pid=`curl -s --cookie-jar cj.txt -X POST 'http://localhost:8020/login' -d 'username=john&password=password' -c - | curl -s -b @- -X POST -H 'Content-Type: application/json' 'http://localhost:8020/api/metaroot.rcv' -d '{}' | jq -r '.id'`
